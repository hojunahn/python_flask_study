...
def classify_place_types(place_types, place_name=None):
    ...
    if place_name:
        ...
        korean_keywords = {
            ...
            '마트/편의점': ['마트', '할인점', '마켓', 'gs25', '씨유','CU', '스톱', '일레븐', '몰','지에스'],
            '쇼핑': ['쿠팡','수입','세외','인터넷'],
            '보험': ['보험', '화재','생명'],
            '카페.간식': ['카페', '커피', '다방', '스타벅스', '떡', '할인점', '브레드', 'bread','바게뜨','뚜레쥬르','프레소'],
            '회사': ['주식회사', '(주)', '기업', '유통', '전자'],
            '운동': ['짐', '헬스'],
            '취미.여가': ['노래', '장'],
            '미용': ['헤어'],
...
def process_excel(file_path):
    ...
    if '가맹점명' in df.columns:
        df['업종'] = df['가맹점명'].map(lambda x: classify_place_types(get_place_type(x), place_name=x) if pd.notna(x) else '기타')
    else:
        raise ValueError("'가맹점명' 열이 데이터에 없습니다.")
    ...
    if '매출금액' in df.columns:
        ...
        raise ValueError("매출금액 또는 거래금액 열이 데이터에 없습니다.")

    # '매출금액' 또는 '거래금액' 열을 숫자로 변환
    df[amount_col] = pd.to_numeric(df[amount_col], errors='coerce')

    기타_df = df[df['업종'] == '기타']
    # 업종과 매출금액 정보 출력
    for idx, row in 기타_df.iterrows():
        print(f"가맹점명: {row['가맹점명']}, 업종: {row['업종']}, {amount_col}: {row[amount_col]}")
